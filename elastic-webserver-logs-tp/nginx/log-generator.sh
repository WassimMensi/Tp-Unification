...
generate_nginx_log() {
    ...
    local user="-"
    local auth="-"
    local time=$(date -u +"%d/%b/%Y:%H:%M:%S +0000")
    ...
    echo "$ip - $auth [$time] \"$method $url $version\" $status $size \"$referer\" \"$user_agent\" \"$forwarded\""
}

# Main loop to generate logs
echo "Starting Nginx log generator..."
while true; do
    # Generate between 1-5 log entries
    num_entries=$((RANDOM % 5 + 1))
    for ((i=0; i<num_entries; i++)); do
        log_entry=$(generate_nginx_log)
        echo "$log_entry" >> $NGINX_LOG_DIR/access.log
    done
